...
import GuesterAutoLogin from './components/account/Guester'
...
const RepositoryGuester = (props) => (
  <Bundle load={cb => require.ensure([], require => cb(require('./components/editor/RepositoryGuester')))}>
    {CustomComponent => CustomComponent ? <CustomComponent {...props} /> : null}
  </Bundle>
)
...
const Routes = ({ match, location }, { store }) => {
  ...
  if (!auth.id) { // 引导用户登陆，已经在 src/index 中实现。这里的代码用于支持没有接入 SSO 的情况。
    ...
    if (winHref.indexOf('/organization/repository/guester') > -1) { // 当为游客模式时，执行的 路由控制
      // window.location.href = ''
      return (
        <Switch>
          <Route component={GuesterAutoLogin} />
        </Switch>
      );
    } else {
      return (
        <article>
          ...
          <Switch>
            ...
            <Route path='/account/guester/:id' component={GuesterAutoLogin} />
  ...
  const HeaderElem = auth.email === 'guest@317hu.com' ? null : <Route component={Header} />;
  return (
        ...
        <Switch>
          ...
          <Route path='/organization' children={() => (
            <Switch>
              ...
              <Route exact path='/organization/repository/guester' component={RepositoryGuester} />
              <Route component={NoMatch} />
            </Switch>
          )} />
